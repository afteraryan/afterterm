...
src/
  ...
  renderer/
    ...
    threadView.ts                      ← Pure: thread kind/state/title, threadName, model label, foldThreads, counter pills, kindWord, runningLabel, needsCloseConfirm, needsSleepConfirm, threadState, projectCounts, threadFolder. Unit-tested.
    ...
    threadMenu.tsx                     ← Thread menu (sidebar right-click, header dots): Open, Sleep/Wake, Move to project, Open localhost:port, Close, Mark as unread/read, Open in File Explorer.
    ...
    components/
      ...
      Header/
        index.tsx                      ← Main pane header: kind icon, name, project/model/branch/worktree line, state chip, dots menu (Sleep or Wake, Open localhost:port); worktree opens the folder in Explorer.
