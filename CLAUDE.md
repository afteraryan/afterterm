...
src/
  ...
  session-files.ts                     ← Edited files: parses transcript lines into changed files, edit events, pasted images and command windows (isBulkCommand); fileKind, normalizePath, pathKey, isInside, mergeChanged, sessionFilesView, ownsTempFile. Pure, unit-tested. Test file: session-files.test.ts
  session-files-store.ts               ← Incremental transcript reader for main: a byte offset per file (the session and its subagents), chunked with yields, pastedBytes. Unit-tested. Test file: session-files-store.test.ts
  file-index.ts                        ← File links' last resort: buildNameIndex (a folder's files by name, skipping what the folder watch skips, capped) and findByName. Unit-tested. Test file: file-index.test.ts
  command-files.ts                     ← Edited files Phase 2: isIgnoredChange, attributeCommandChanges (a folder change counts only inside one of the chat's command windows), pruneChanges, mergeSaved. Pure, unit-tested. Test file: command-files.test.ts
  ...
  renderer/
    ...
    filesView.ts                       ← Pure: the Files button's label and list (docs, code, pasted), folder labels relative to the chat then the project, short times. Unit-tested. Test file: filesView.test.ts
    filePaths.ts                       ← Pure: file paths in terminal text: findPathCandidates, resolveCandidates, matchChangedName, continuation, elidedTail, openKind. Unit-tested. Test file: filePaths.test.ts
    ...
    threadMenu.tsx                     ← Two thread menus over shared items, no Open item in either: buildThreadMenu (sidebar right-click, project page rows) and buildHeaderMenu (header dots, no editor entries). Sleep/Wake, Mark as unread/read, Move to project, Open localhost:port, Open project page, Open in File Explorer, Open in <editor>, Close.
    ...
    components/
      ...
      JumpButton/
        index.tsx                      ← Flipping jump button: 32px chevron centred in its scroller; mousedown prevented so it never steals focus; wheel passes through via onWheel; hides JUMP_IDLE_MS after the host's last poke() unless the pointer moved onto it after the scroll paused.
      ...
      Terminal/
        index.tsx                      ← xterm.js lifecycle, PTY wiring, title intelligence, OSC 9;9/OSC 7 cwd capture, clipboard/links/find bar/font zoom/drag-drop, teardownTerminal/createTerminal, the OSC 133 handler, pendingDestroyRef/creatingRef, harness hooks (fileLinks, openFileLink, findRow), JumpButton wiring, the file-link provider.
        fileLinks.ts                   ← Edited files Phase 3: the xterm link provider for file paths (buffer cells to text, batched files:stat with a cache, tool-line time markers, the hover note). fileLinks.css styles the note.
      FilesButton/
        index.tsx                      ← Edited files: the header's Files button and its list (Documents, the Code and pasted folds, thumbnails, right-click menu with the editor logo and Explorer icon, Esc and outside click). FilesButton.css holds the prototype's look and motion.
      Icons.tsx                        ← SVG icons: FolderIcon, StateIcon, KindIcon, Spinner, ProjectIcon (ten glyphs), IconCollapseAll/IconExpandAll, IconCompact, IconFiles/IconDoc/IconCode/IconImage/IconFoldChev.
...
scripts/
  agent-harness/
    ...
    drive.mjs                          ← CDP client: targets, bounds, screenshot, eval, dom, click, rightclick, hover, unhover, drag, emulate-media, type, key, sidebar, rail, dock, search, screen, home, project, chooser, palette, header, hover-card, pane, tail, confirm, opened, toasts, marks, counts, reload, window, scroll, jump, pane-scroll, record.
...
**The rail and the panel.** The rail (`components/Rail`) is always on, on every screen: Home and Workspace, then a tile per project that has a thread waiting, working, finished or compacting, with badges beside it. The panel (`components/SidePanel`) lists General, Pinned and Recent with collapse buttons on the headings, an in-place search box, a five-row fold per project that stays open while a hidden row is waiting, and a docked Other projects row; it can be hidden (`ui.panelHidden`, Ctrl+Shift+B). Ctrl+Shift+Down/Up cycle thread rows in panel order across projects; Ctrl+Tab cycles in session order. A new or resumed thread goes first in its project (`insertNewThread`), so the fold shows the newest. Opening a project from Home, the rail, the palette or the Other projects drawer scrolls the panel to it and highlights its row (`revealProject`, `revealScrollTop`). The thread menus are `threadMenu.tsx` (the row menu and the header's own), the one project menu `projectMenu.tsx`. `docs/features-attention-and-sidebar.md`.

**Edited files.** A chat's header has a Files button (`components/FilesButton`) listing the documents it changed first, then its code and the images pasted into it, each folded to a count. Main reads the session's transcript and its subagents' files incrementally (`session-files-store.ts`, `session-files.ts`) on every turn and on view; files Claude changed with shell commands come from a folder watch attributed to the chat's own command windows (`command-files.ts`) and are saved per session in `<userData>\edited-files\`. File paths in the terminal are links (`Terminal/fileLinks.ts`, `filePaths.ts`), underlined only once main says they exist; since 2026-09-26 any file name with an extension links, found among the files the chat touched, then in its folders, then by name under them (`file-index.ts`, `files:find`). Everything is in `docs/edited-files/` (design, phases and status, build notes, testing); that feature's status is there, not in `PHASES.md`.
...
- `docs/edited-files/`: the Files button and clickable file paths: `design-04-edited-files.md` (what was agreed), `phases.md` (status, decisions, log), `build-notes.md`, `testing-and-handoff.md`
