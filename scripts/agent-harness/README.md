...
| `sidebar` | `drive sidebar` | The rendered sidebar as a tree: groups with label, folder, tab count, collapsed state and badge; tabs with title, `*` for active, `[restorable]`, `[working]`, `[<notification>]`; the Projects shelf rows if present. |
