...
| `drive.mjs` | CDP client with subcommands: `targets`, `bounds`, `screenshot`, `eval`, `dom`, `click`, `rightclick`, `hover`, `unhover`, `drag`, `emulate-media`, `type`, `key`, `sidebar`, `rail`, `dock`, `search`, `screen`, `home`, `project`, `chooser`, `palette`, `header`, `hover-card`, `pane`, `tail`, `confirm`, `opened`, `marks`, `jump`, `pane-scroll`, `counts`, `reload`, `window`, `record`. |
...
| `jump` | `drive jump` | Phase 9: the jump-to-top / jump-to-bottom button (`.jump-btn`, `src/renderer/components/JumpButton/index.tsx`), mounted over either scroller it appears on (the live terminal or the asleep pane). Prints `jump: hidden` when no button is in the DOM, or `jump: shown target=<top\|bottom> tip="<text>"` per button found (there should only ever be one, since only the active tab's scroller shows it, but every match prints if that ever stops holding). |
