...
- 2026-09-26: Aryan could not open images an agent sent: agents name them bare in replies ("03-operator-home.png"), and a bare name only linked to a changed file, which images never are. Changed on his word: every file name with an extension links. Order for a bare name: files the chat touched (changed, written including images, read, or sent with SendUserFile; newest), then the chat's and project's folders, then a name index of those folders (`src/file-index.ts`, main's `files:find`, rebuilt after 30s, skipping what the folder watch skips; the nearest match). A path with folders tries the folders first. The Files list is unchanged (images Claude made still stay out of it). Self-tested on the scratch chat: an SVG it wrote, a screenshot and two `burger.png` files nothing had touched, `v4/burger.png`, `README.md` and `notes.md` all linked and opened the right files (screenshot 01 and recording 01 in `docs/screenshots/file-links-any-extension/`). Branch `file-links-any-extension`.
- 2026-09-26: a path starting with a dot folder (`.temp/docs/x.html`, `.claude/commands/bug-record.md`) never linked: the recogniser stripped leading punctuation including that dot, so it looked for `temp\docs\x.html`. Now only sentence punctuation and a real ellipsis are stripped. Found by Aryan in a chat's reply; tested in a shell in the harness (screenshot 02, recording 02 in `docs/screenshots/file-links-any-extension/`).
